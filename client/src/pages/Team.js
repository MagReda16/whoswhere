...
function Team () {
  ...
  const authContext = useAuth();
    ...
    const matchingTeam = usersArray.filter(user => user.team === currentUserTeam)
  ...
  return (
    <div>
      <h2><span className="team_name">{currentUserTeam}</span> Team</h2>
      ...
      <div className="task_wrapper">
        ...
        <button className="show_tasks" onClick={handleClick}> {taskDisplay ? "Hide Tasks" : "Show My Tasks"}</button>
          <div className="task_list" style={{display: taskDisplay ? 'block' : 'none' }}>
            {adminTasks.length > 0 ?
              adminTasks.map(task => {
              return <Tasks 
              key={task}
